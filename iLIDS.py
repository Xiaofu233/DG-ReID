...
class iLIDS(ImageDataset):
    ...
    def parse_img_names(self, img_names, pid2label, train_label=None):
        data = []
    ...
    def process_split(self, split):
        imgs = split['train'] + split['query'] + split['gallery']
        pid2label = self.get_pid2label(imgs)
        train = self.parse_img_names(split['train'], pid2label, train_label=True)
        query = self.parse_img_names(split['query'], pid2label)
        gallery = self.parse_img_names(split['gallery'], pid2label)
